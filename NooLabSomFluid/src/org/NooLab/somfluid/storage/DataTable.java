...
import org.NooLab.somfluid.structures.DataTableRow;
...
if (r>rc-3){
	...
	private boolean hasHeader() {
		...
		// there is the case that the "RowValuesArrayList" is numeric, but header is available anyway
		if (rB==false){
			int cc, ch ;
			
			cc = colcount ;
			ch = columnHeaders.size() ;
			
			if ((headerAvailable) && (cc>1) && (cc==ch) && (columnHeaders.get(columnHeaders.size()-1).length()>0)){
				rB=true;
			}
		}
	...
	public   ArrayList<Double> getDataTableRow(int rIndex) {
		
		ArrayList<Double> rowData = new ArrayList<Double> ();
		
		if ((rIndex>=0) && (rIndex< this.dataTableRows.size())){
			rowData = dataTableRows.get( (int)Math.round(1.0*rIndex)); 
		}
	...
	private void setRowCount(int numberofRows){
	...
	public ArrayList<Double> getRowValuesArrayList(long ix){
		...
		if ((this.dataTableRows != null) && (dataTableRows.size() >= dataTable.get(0).rowcount-2) && (ix<dataTableRows.size())) {
			numRowArrayList = dataTableRows.get( (int)Math.round(1.0*ix) ) ;
		} else {
			for (int i = 0; i < colcount; i++) {
				val = dataTable.get(i).getValue( (int)Math.round(1.0*ix) );
