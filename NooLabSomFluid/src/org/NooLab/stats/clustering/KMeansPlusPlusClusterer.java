...
import org.NooLab.stats.exception.NumberIsTooSmallException;
import org.NooLab.stats.util.MathUtils;
...
public class KMeansPlusPlusClusterer<T extends Clusterable<T>> {
    ...
    static int[] inactivateColumns = new int[0] ;
    ...
    public List<Cluster<T>> cluster(final Collection<T> points, final int k,
        ...
        int n=this.inactivateColumns.length;
        ...
        for (int count = 0; count < max; count++) {
            ...
            for (final Cluster<T> cluster : clusters) {
                ...
                if (cluster.getPoints().isEmpty()) {
                    ...
                    newCenter = cluster.getCenter().centroidOf(cluster.getPoints(), missingValue);
                }
                newClusters.add(new Cluster<T>(newCenter));
            }
            int changes = assignPointsToClusters(newClusters, (Collection<T>)advData.getRawPoints(), assignments);
            clusters = newClusters;							// points
     ...
     */
    private <T extends Clusterable<T>> int
        		   assignPointsToClusters(  final List<Cluster<T>> clusters, 
        				   						  Collection<T> points,
     ...
     */
    private <T extends Clusterable<T>> List<Cluster<T>>
        ...
        for (int i = 0; i < numPoints; i++) {
            if (i != firstPointIndex) { // That point isn't considered
                double d = firstPoint.distanceFrom( advData.getPoint(i),useIndicator, missingValue);
            ...
            if (nextPointIndex >= 0) {
                ...
                if (resultSet.size() < k) {
                    ...
                    for (int j = 0; j < numPoints; j++) {
                        ...
                        if (!taken[j]) {
                            double d = p.distanceFrom( advData.getPoint(j),useIndicator, missingValue);
    ...
    private T getPointFromLargestVarianceCluster(final Collection<Cluster<T>> clusters) {
        ...
        for (final Cluster<T> cluster : clusters) {
            if (!cluster.getPoints().isEmpty()) {
                ...
                for (final T point : cluster.getPoints()) {
                    stat.increment(point.distanceFrom(center,useIndicator, missingValue));
                }
    ...
    private T getFarthestPoint(final Collection<Cluster<T>> clusters) {
        ...
        for (final Cluster<T> cluster : clusters) {
            ...
            for (int i = 0; i < points.size(); ++i) {
                final double distance = advData.getPoint(i).distanceFrom(center,useIndicator, missingValue);
    ...
    private <T extends Clusterable<T>> int getNearestCluster(final Collection<Cluster<T>> clusters, final T point) {
        ...
        for (final Cluster<T> c : clusters) {
            final double distance = point.distanceFrom( c.getCenter(),useIndicator, missingValue );
    ...
    private  void defineUseIndicators( int colcount){
    	...
    	for (int i=0;i<inactivateColumns.length;i++){
    		if ((inactivateColumns[i]>=0) && (inactivateColumns[i]<colcount)){
    			useIndicator[ inactivateColumns[i] ] = 0; // 
    		}
    	}
    }
