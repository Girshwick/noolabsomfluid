...
public class CyclePathImpl extends PathImpl implements CyclePath {
    ...
    /**
     * Adds a Vertex into the Path. The check whether or not
     * the resulting <tt>Path</tt> is still simple and non-cycle
     * is done on subsequent calls to <tt>add( Vertex )</tt> or
    ...
    public void add( Vertex newVertex ) throws Exception {
        ...
        // Raise an exception if the last Vertex added to the path
        ...
        this.setFlags( newVertex );

        // Let PathImpl handle the actual adding of the Vertex to the Path.
        super.add( newVertex );
    }
     ...
     * <tt>add( Vertex )</tt> or when <tt>closeCycle()</tt> is called.
     *
    ...
    public void closeCycle() throws IllegalPathException {
        Vertex  lastVertex = ( Vertex ) this.vertexStack.peek();
    ...
    private void validatePath() throws IllegalPathException {
        // If the last Vertex added formed a cycle, raise the exception
        if( this.lastVertexFormedCycle ) {
            throw new IllegalPathException( "Path is already a cycle but path is not closed." );
        }

        // If the last Vertex added change the path to become non-simple,
        // raise the exception
     ...
     *
     * @param   newVertex   The <tt>Vertex</tt> that is intended to be added to
     * the <tt>CyclePath</tt>.
     */
    private void setFlags( Vertex newVertex ) {
        ...
        // of the flags will be read on the next add( Vertex )
        // or on closeCycle().
        if( !this.vertexStack.empty() &&
            (((Vertex) this.vertexStack.get( 0 ) ) == newVertex )) {
            this.lastVertexFormedCycle = true;
        }

        if( this.vertexStack.contains( newVertex )) {
