...
import org.NooLab.graph.commons.DirectedAcyclicGraphImpl;
import org.NooLab.graph.commons.DirectedEdgeImpl;
import org.NooLab.graph.commons.DirectedGraphImpl;
import org.NooLab.graph.commons.DirectedWeightedEdgeImpl;
import org.NooLab.graph.commons.EdgeIntf;
import org.NooLab.graph.commons.Edge;
import org.NooLab.graph.commons.OpenJGraphFactory;
...
import org.NooLab.graph.commons.WeightedEdgeImpl;
import org.NooLab.graph.commons.WeightedGraph;
import org.NooLab.graph.visual.*;
import org.NooLab.graph.visual.drawing.Painter;
import org.NooLab.graph.visual.layout.GraphLayoutManager;
...
/**
 * Custom ContentHandler for parsing an XGMML input source using SAX2.
 *
...
public class XGMMLContentHandler extends XMLToGraphHandler {
    ...
    public void instantiateGraph() throws Exception {
        if( this.directedAcyclic )
            ...
            this.graph = new OpenJGraph();

        // Set the GraphFactory used by the Graph
        if( this.graphFactoryClassName != null ) {
            OpenJGraphFactory factory = ( OpenJGraphFactory ) Class.forName( this.graphFactoryClassName ).newInstance();
    ...
    public void startGraphElement( Attributes attribs ) {
        if( attribs.getValue( XGMML.DIRECTED_ATTRIBUTE_LITERAL ).equals( "1" ))
            this.directed = true;
        if( attribs.getValue( XGMML.GRAPHIC_ATTRIBUTE_LITERAL ).equals( "1" ))
            this.graphic = true;
    }
    ...
    public void instantiateEdge() throws Exception {
        ...
        if ( ( this.directed || this.directedAcyclic ) && !this.weighted )
            ...
            this.edge = new Edge( this.vertexA, this.vertexB );
        }
