        ...
        while( !iterator.isDone() ){
            ...
            if( cumulativeLength >= ( edgeLength / 2 ) || cumulativeLength >= edgeLength ) {
                ...
                if( this.fontMetrics == null ) {
                    this.fontMetrics = new Panel().getFontMetrics( vEdge.getFont() );
                }

                // Take into account the text's length
                this.paintText(g2d, vEdge.getFont(), vEdge.getFontcolor(), vEdge.getText(),
                    (float) (fromPoint.getX() < toPoint.getX() ?
                        (x1 + ( Math.abs( x2 - x1 ) * ratio )):
                        (x1 - ( Math.abs( x2 - x1 ) * ratio )))
                            - fontMetrics.stringWidth( vEdge.getText() ) / 2,
