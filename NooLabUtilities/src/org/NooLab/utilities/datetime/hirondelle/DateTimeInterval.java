...
import static org.NooLab.utilities.datetime.hirondelle.DateTime.DayOverflow;
import static org.NooLab.utilities.datetime.hirondelle.DateTime.Unit;
...
final class DateTimeInterval {
  ...
  private void checkRange(Integer aValue, String aName) {
    if (checkrange(aValue, MIN, MAX)) { 
      throw new IllegalArgumentException(aName + " is not in the range " + MIN + ".." + MAX); 
    }
  }
  
  private boolean checkrange(Integer aValue, int min, int max) {
	boolean rB;
