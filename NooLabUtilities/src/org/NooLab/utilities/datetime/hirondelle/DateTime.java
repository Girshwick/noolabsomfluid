...
import org.NooLab.utilities.datetime.hirondelle.TimeSource;
import org.NooLab.utilities.datetime.hirondelle.TimeZoneSource;
import org.NooLab.utilities.datetime.hirondelle.ModelUtil.NullsGo;
 ...
 */
  ...
  private void checkRange(Integer aValue, int aMin, int aMax, String aName) {
    if (!checkrange(aValue,aMin, aMax)) {
      throw new ItemOutOfRange(aName + " is not in the range " + aMin + ".." + aMax + ". Value is:" + aValue);
    }
  }

  private boolean checkrange(int aValue, int min, int max) {
	boolean rB;
	
	rB = (min<=aValue) && (max>=aValue) ;
	return rB;
}
