...
if ( ((xpos>430) && (xpos<460))	&& ((ypos<50))){
	...
	ArrayList<IndexDistance> getAdjustedSurroundSelection( int xpos, int ypos, 
	...
	public ArrayList<IndexDistance> getItemsOfSurround(int index, int surroundSize, int expectedCollSize, String guidStr) {
		...
		if ((xs==expectedCollSize) && ( bcix )){
			 
			if ((surroundBuffers!=null) && (surroundBuffers.bufferIsAvailable( index, surroundSize)) && (surroundBuffers.getBufferingSwitchedOff()==false)){
				...
				out.print(3, "\nbuffer used for particle "+index);
			...
			if(surroundBuffers.getBufferingSwitchedOff()==false){
				out.print(3, "\nbuffer for particle "+index+" is not available, direct retieval will be started");
			}
			   //  (int)
...
class _Plane_ {
	...
	public _Plane_(Neighborhood nb, String name) {
		neighborhood = nb;
		this.name = name;
	...
	/**
	 * called from main part of Neighborhood object , from within "getItemsOfSurround()"
	 * 
	 * @param index
	 * @param surroundradius
	 * @param guidStr
	 * @return
	 */
	...
	public void update( Item item, String name) {
	...
	public void sort(int direction, Coordinate2D triggeringItem) {
		int p,index;
		Object tItem;
		Object[] items;
		Coordinate cc;
