...
public class Surround {
	...
	public Surround(RepulsionField parent){
		
		parentField = parent;
		neighborhood = parentField.getNeighborhood() ;
		
		
		surroundBuffers = parentField.getSurroundBuffers();
	...
	public int[] getGeometricSurround(int xpos, int ypos, int surroundN, int shape) {
		...
		out.print(3, "now retrieving surround for particles of index = "+index+"...") ;
		...
		if ((particleIndexes!=null) &&(particleIndexes.length > 0)) {
			...
			out.print(3, "retrieving surround finished.") ; 
			ctime= System.currentTimeMillis();
			out.print(2, "a group of "+particleIndexes.length+" particles have been retrieved in "+(ctime-stime)+" ms");
	...
	public int[] getGeometricSurround(int index, int surroundN, int shape) {
		...
		{      
			...
			if (indexedDistance != null) {
				particleIndexes = new int[indexedDistance.size()];
				particleDistances = new double[indexedDistance.size()];

				out.print(4, "transposing results from <indexedDistance> to <particleIndexes> ...  ");

				int k = indexedDistance.size();
				...
				for (int i = 0; i < k; i++) {
					...
					particleIndexes[i] = index;
					particleDistances[i] = indexedDistance.get(i).getDistance();
				}
