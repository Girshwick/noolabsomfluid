...
public class RepulsionFieldApplet  extends PApplet implements RepulsionFieldEventsIntf {
	...
	private void initializeRepulsionField() { 
		...
		// repulsionField.init(repulsionField, "importField" );
	...
	public void keyPressed(){
		...
		if (key=='A'){
			mouseIsActive = false;
			requestGuid = repulsionField.addParticles( mouseX,mouseY);
		}
		if (key=='a'){
			mouseIsActive = false;
			requestGuid = repulsionField.addParticles(1);
		}
		if (key=='d'){
			mouseIsActive = false;
			requestGuid = repulsionField.deleteParticle( (int)(Math.random()*(repulsionField.getNumberOfParticles() )));
		}
		if (key=='r'){
			mouseIsActive = false;
	...
	private void drawparticle(int index){
		...
		try{
			...
			if (selectedSet.indexOf( index)>=0){
				r = 230;g=50;b=50;
			}
			fill(r, g, b);
